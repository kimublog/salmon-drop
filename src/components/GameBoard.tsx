...
export default function GameBoard({
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[100dvh] bg-gradient-to-b from-[#0D3B66] to-[#1B4F72] text-white p-2 sm:p-4 overflow-hidden">
      {/* 上部: 難易度 + スコア + 一時停止（モバイル横並び） */}
      <div className="flex items-center gap-3 mb-2 w-full max-w-md justify-center lg:hidden">
        <div className="px-3 py-1 bg-white/10 rounded-lg text-xs">
          {difficultyName}
        </div>
        <div className="text-center px-3 py-1 bg-white/10 rounded-lg">
          <span className="text-xs text-blue-300 mr-1">SCORE</span>
          <span className="text-lg font-bold text-orange-300">{score.score.toLocaleString()}</span>
        </div>
        <div className="text-center px-3 py-1 bg-white/10 rounded-lg">
          <span className="text-xs text-blue-300 mr-1">連鎖</span>
          <span className="text-lg font-bold">{score.maxChain}</span>
        </div>
        <button
          onClick={isPaused ? onResume : onPause}
          className="px-3 py-1 bg-white/20 rounded-lg text-xs"
        >
          {isPaused ? "再開" : "||"}
        </button>
      </div>
      ...
      <div className="flex flex-col lg:flex-row items-center justify-center gap-4">
        ...
        <div className="relative">
          <canvas
            ...
            width={BOARD_WIDTH}
            height={BOARD_HEIGHT}
            className="rounded-xl shadow-2xl border-2 border-white/20 max-w-[90vw] max-h-[55dvh] lg:max-h-[80vh]"
            style={{
              width: BOARD_WIDTH,
              height: BOARD_HEIGHT,
              objectFit: "contain",
      ...
      <div className="lg:hidden mt-3 flex flex-col items-center gap-2 w-full max-w-sm select-none">
        {/* 操作説明 */}
        <p className="text-xs text-blue-300/70 mb-1">同じサーモンを4つ以上つなげて消そう</p>
        ...
        <div className="flex items-center gap-2 w-full justify-center">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="w-16 h-16 bg-white/15 rounded-2xl text-2xl active:bg-white/40 flex flex-col items-center justify-center touch-manipulation"
            >
              <span className="text-xl">←</span>
              <span className="text-[10px] text-blue-300">左</span>
            </button>
            <button
            ...
            >
              <span className="text-xl">→</span>
              <span className="text-[10px] text-blue-300">右</span>
            </button>
          </div>
          ...
          >
            <span className="text-xl">↻</span>
            <span className="text-[10px] text-blue-200">回転</span>
          </button>
          ...
          <div className="flex gap-2">
            <button
              ...
              className="w-16 h-16 bg-white/15 rounded-2xl active:bg-white/40 flex flex-col items-center justify-center touch-manipulation"
            >
              <span className="text-xl">↓</span>
              <span className="text-[10px] text-blue-300">下</span>
            </button>
            <button
